...
print(f"[{datetime.now()}] 开始 stk_pledge...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'stk_pledge' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'stk_pledge')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] stk_pledge 完成")
