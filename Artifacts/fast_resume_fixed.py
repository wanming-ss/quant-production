...
def fast_resume(api_name, csv_name, rename_cols, id_col='symbol'):
    ...
    if os.path.exists(CSV_PATH):
        try:
            # 尝试读取，跳过错误行
            existing_df = pd.read_csv(CSV_PATH, low_memory=False, on_bad_lines='skip')
            if id_col in existing_df.columns:
                completed_stocks = set(existing_df[id_col].unique())
            elif 'ts_code' in existing_df.columns:
                completed_stocks = set(existing_df['ts_code'].unique())
            elif 'symbol' in existing_df.columns:
...
if __name__ == "__main__":
    ...
    if api == 'weekly':
        ...
        fast_resume('moneyflow', 'tushare_moneyflow.csv', {'ts_code': 'ts_code', 'trade_date': 'trade_date'}, 'ts_code')
    else:
        print(f"未知: {api}")
