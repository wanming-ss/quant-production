...
print(f"[{datetime.now()}] 开始 stk_reward...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'stk_reward' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'stk_reward')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] stk_reward 完成")
