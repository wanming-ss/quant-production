...
CSV_PATH = "E:/Quant_Production/Inbox/tushare_holdertrade.csv"

print(f"[{datetime.now()}] 开始 stk_holdertrade...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'stk_holdertrade' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'stk_holdertrade')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] stk_holdertrade 完成")
