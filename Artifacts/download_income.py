...
CSV_PATH = "E:/Quant_Production/Inbox/tushare_income.csv"

print(f"[{datetime.now()}] 开始 income...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'income' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'income')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] income 完成")
