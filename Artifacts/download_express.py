...
CSV_PATH = "E:/Quant_Production/Inbox/tushare_express.csv"

print(f"[{datetime.now()}] 开始 express...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'express' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'express')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] express 完成")
