...
print(f"[{datetime.now()}] 开始 top_inst...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'top_inst' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'top_inst')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] top_inst 完成")
