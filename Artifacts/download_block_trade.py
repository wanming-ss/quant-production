...
CSV_PATH = "E:/Quant_Production/Inbox/tushare_block_trade.csv"

print(f"[{datetime.now()}] 开始 block_trade...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'block_trade' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'block_trade')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] block_trade 完成")
