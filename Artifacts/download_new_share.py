...
CSV_PATH = "E:/Quant_Production/Inbox/tushare_new_share.csv"

print(f"[{datetime.now()}] 开始 new_share...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'new_share' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'new_share')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] new_share 完成")
