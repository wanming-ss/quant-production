...
CSV_PATH = "E:/Quant_Production/Inbox/tushare_holdernumber.csv"

print(f"[{datetime.now()}] 开始 stk_holdernumber...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'stk_holdernumber' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'stk_holdernumber')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] stk_holdernumber 完成")
