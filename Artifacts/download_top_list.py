...
print(f"[{datetime.now()}] 开始 top_list...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'top_list' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'top_list')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] top_list 完成")
