...
print(f"[{datetime.now()}] 开始 fina_indicator...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'fina_indicator' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'fina_indicator')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] fina_indicator 完成")
