...
CSV_PATH = "E:/Quant_Production/Inbox/tushare_forecast.csv"

print(f"[{datetime.now()}] 开始 forecast...")
...
for j, (_, row) in enumerate(stock_df.iterrows(), 1):
    ...
    try:
        if 'forecast' == 'limit_list':
            continue  # skip
        else:
            df = getattr(pro, 'forecast')(ts_code=ts_code, start_date='20160101', end_date='20250324')
...
print(f"[{datetime.now()}] forecast 完成")
