...
print("="*70)
print("导入最后新增数据到 DolphinDB (884万条)")
print("="*70)
...
# 4. 导入（数据量小，一次性导入）
print(f"\n3️⃣  导入 {new_records:,} 条...")

batch_df = pd.read_csv(CSV_PATH, skiprows=range(1, current_count + 1))
batch_df.columns = ['symbol', 'date', 'open', 'high', 'low', 'close', 'pre_close', 'change', 'pct_chg', 'volume', 'amount']

temp_csv = "E:/Quant_Production/Inbox/temp_final.csv"
...
try:
    ...
    print(f"   ✅ 完成 {len(batch_df):,} 条，总计 {imported.values[0][0]:,}")
