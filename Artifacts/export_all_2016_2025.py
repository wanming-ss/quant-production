#!/usr/bin/env python3
"""
Tushare 全量股票数据导出 (2016-2025)
5491只股票，约10年历史
"""
import pandas as pd
import tushare as ts
from datetime import datetime
import time

TOKEN = "5bb803b4f1bdc5ed7762f89d9109a809"
URL = "http://119.45.170.23"
CSV_PATH = "E:/Quant_Production/Inbox/tushare_all_2016_2025.csv"
BATCH_SIZE = 100  # 每批保存100只股票

# 初始化
pro = ts.pro_api(TOKEN)
pro._DataApi__token = TOKEN
pro._DataApi__http_url = URL

print("="*70)
print("Tushare 全量股票数据导出 (2016-2025)")
print("="*70)
print(f"开始时间: {datetime.now()}")

# 获取所有股票
print("\n📊 获取股票列表...")
stock_df = pro.stock_basic(exchange='', list_status='L', fields='ts_code,name')
total_stocks = len(stock_df)
print(f"📝 总计 {total_stocks} 只股票")

# 分批处理
all_data = []
total_records = 0
processed = 0

for i, (_, row) in enumerate(stock_df.iterrows(), 1):
    ts_code = row['ts_code']
    name = row['name']
    
    # 每10只显示进度
    if i % 10 == 1 or i == total_stocks:
        print(f"\n[{i}/{total_stocks}] {ts_code} {name}")
    
    try:
        # 获取 2016-2025 数据
        df = pro.daily(
            ts_code=ts_code,
            start_date='20160101',
            end_date='20250324'
        )
        
        if df is not None and not df.empty:
            df = df.rename(columns={
                'ts_code': 'symbol',
                'trade_date': 'date',
                'vol': 'volume'
            })
            df['date'] = pd.to_datetime(df['date'], format='%Y%m%d')
            all_data.append(df)
            total_records += len(df)
            processed += 1
            
            if i % 10 == 0:
                print(f"   ✅ {len(df)} 条 (累计 {total_records:,})")
        
    except Exception as e:
        print(f"   ❌ {str(e)[:50]}")
    
    # 请求间隔
    time.sleep(0.08)
    
    # 每 BATCH_SIZE 保存一次（防止内存溢出）
    if i % BATCH_SIZE == 0 and all_data:
        batch_df = pd.concat(all_data, ignore_index=True)
        
        # 追加到 CSV
        if i == BATCH_SIZE:
            batch_df.to_csv(CSV_PATH, index=False, mode='w')
            print(f"\n💾 首次保存 {len(batch_df):,} 条")
        else:
            batch_df.to_csv(CSV_PATH, index=False, mode='a', header=False)
            print(f"\n💾 追加保存 {len(batch_df):,} 条 (累计 {total_records:,})")
        
        all_data = []  # 清空内存
        print(f"📊 进度: {i}/{total_stocks} ({i/total_stocks*100:.1f}%)")

# 保存剩余数据
if all_data:
    batch_df = pd.concat(all_data, ignore_index=True)
    if total_stocks <= BATCH_SIZE:
        batch_df.to_csv(CSV_PATH, index=False, mode='w')
    else:
        batch_df.to_csv(CSV_PATH, index=False, mode='a', header=False)
    print(f"\n💾 最终保存 {len(batch_df):,} 条")

print(f"\n" + "="*70)
print("✅ 全量导出完成")
print(f"CSV: {CSV_PATH}")
print(f"成功股票: {processed}/{total_stocks}")
print(f"总记录: {total_records:,}")
print(f"结束时间: {datetime.now()}")
print(f"\n预计 CSV 大小: ~{total_records * 100 / 1024 / 1024:.1f} MB")
print("="*70)
